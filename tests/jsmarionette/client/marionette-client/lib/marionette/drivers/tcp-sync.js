...
function TcpSync(options) {
  ...
  this.connectionTimeout = options.conneectionTimeout || 2000;
...
TcpSync.prototype._readResponse = function() {
  ...
  stream.on('data', function(parsed) {
    ...
    console.log("stream on data=" + util.inspect(parsed));
    */
    data = parsed;
  });
  ...
  if (Array.isArray(data) &&
      data.length == 4 &&
      data[0] == Response.TYPE) {
    var resp = Response.fromMsg(data);
    res = resp.error || resp.result;
